...
// This function plays a the game as soon as a winning path is found
import { makeMoveAndReveal } from '../../../src/helper/board-functions'
import {
  cssPositionToIndex,
...
import {
  getTarget,
  revealTalonCard,
} from '../../../src/helper/cypress-functions'
import { Board, Card, Move } from '../../../src/lib/types'
import { simulateGameAndGetMoves } from '../../../src/helper/simulation'
  ...
  it('play game', () => {
    ...
    tableauCards
      ...
      .then(() => {
        ...
        let result = { gameWon: false, playedMoves: [] as Move[] }
        while (simulationsCounter < 1000) {
          const r = simulateGameAndGetMoves(board)
          if (r.gameWon) {
            cy.log('game won')
            result = r
            break
        ...
        cy.log('played moves: ', result.playedMoves)

        if (result.playedMoves.length === 0) {
          cy.log('no moves found')
          console.log('no moves found')
          return
        }

        result.playedMoves.forEach((move) => {
